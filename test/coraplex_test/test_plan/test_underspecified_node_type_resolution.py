...
def test_class_diagram_resolves_underspecified_node():
    ...
    ``ActionDescription`` was imported under ``TYPE_CHECKING`` from the
    ``coraplex.robot_plans`` package, which never re-exported it (and ``coraplex``'s
    ``__init__.py`` files stay import-free, so the fix imports directly from the
    defining submodule instead of adding a package re-export).
    """
