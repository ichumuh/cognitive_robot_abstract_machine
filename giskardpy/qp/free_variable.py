...
class FreeVariable:
    ...
    def __init__(
        ...
        for derivative in Derivatives:
            ...
            self._symbols[derivative] = symbol_manager.register_symbol_provider(
                symbol_name, lambda n=name, d=derivative: god_map.world.state[n][d]
            )
    ...
    def get_symbol(self, derivative: Derivatives) -> Union[cas.Symbol, float]:
        try:
            ...
            raise KeyError(
                f"Free variable {self} doesn't have symbol for derivative of order {derivative}"
            )
    ...
    @memoize
    def get_lower_limit(
        self, derivative: Derivatives, default: bool = False, evaluated: bool = False
    ) -> Union[cas.Expression, float]:
        if (
            not default
            and derivative in self.default_lower_limits
            and derivative in self.lower_limits
        ):
            expr = cas.max(
                self.default_lower_limits[derivative], self.lower_limits[derivative]
            )
            ...
            raise KeyError(
                f"Free variable {self} doesn't have lower limit for derivative of order {derivative}"
            )
        if evaluated:
            if expr is None:
                return None
    ...
    def set_lower_limit(
        self, derivative: Derivatives, limit: Union[cas.Expression, float]
    ):
        self.lower_limits[derivative] = limit

    def set_upper_limit(
        self, derivative: Derivatives, limit: Union[Union[cas.Symbol, float], float]
    ):
    ...
    def get_upper_limit(
        ...
        if (
            not default
            and derivative in self.default_upper_limits
            and derivative in self.upper_limits
        ):
            expr = cas.min(
                self.default_upper_limits[derivative], self.upper_limits[derivative]
            )
            ...
            raise KeyError(
                f"Free variable {self} doesn't have upper limit for derivative of order {derivative}"
            )
        if evaluated:
            if expr is None:
                return None
    ...
    def get_lower_limits(self, max_derivative: Derivatives) -> Dict[Derivatives, float]:
        ...
        for derivative in Derivatives.range(Derivatives.position, max_derivative):
            lower_limits[derivative] = self.get_lower_limit(
                derivative, default=False, evaluated=True
            )
        return lower_limits
    ...
    def get_upper_limits(self, max_derivative: Derivatives) -> Dict[Derivatives, float]:
        upper_limits = {}
        for derivative in Derivatives.range(Derivatives.position, max_derivative):
            upper_limits[derivative] = self.get_upper_limit(
                derivative, default=False, evaluated=True
            )
        return upper_limits
