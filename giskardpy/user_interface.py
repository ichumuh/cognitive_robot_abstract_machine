...
class MonitorWrapper:
    ...
    ]  # node name to start, pause, end, reset
    ...
    def add_monitor(
        ...
        *,
        ...
        self._conditions[monitor.name] = (
            quote_node_names(start_condition),
    ...
    def add_set_seed_configuration(
        ...
        monitor = SetSeedConfiguration(seed_configuration=seed_configuration, name=name)
    ...
    def add_set_seed_odometry(
        ...
        monitor = SetOdometry(base_pose=base_pose, group_name=group_name, name=name)
        return self.add_monitor(
            monitor=monitor,
            start_condition=start_condition,
            pause_condition="",
    ...
    def add_end_motion(self, start_condition: str, name: str = "Done") -> str:
        """
...
class MotionGoalWrapper:
    ...
    def _generate_default_name(
        self, class_type: Union[Type[Task], Type[Goal]], name: Optional[str]
    ):
        if name is None:
            name = f"{class_type.__name__}{len(self._conditions)}"
        return name
    ...
    def add_joint_position(
        ...
        joint_task = JointPositionList(
            name=name, goal_state=goal_state, weight=weight, max_velocity=max_velocity
        )
...
class GiskardWrapper:
    ...
    def compile(self):
        task_state_symbols = (
            god_map.motion_statechart_manager.task_state.get_observation_state_symbol_map()
        )
        monitor_state_symbols = (
            god_map.motion_statechart_manager.monitor_state.get_observation_state_symbol_map()
        )
        goal_state_symbols = (
            god_map.motion_statechart_manager.goal_state.get_observation_state_symbol_map()
        )
        ...
        for name, (start, reset, pause, end) in chain(
            self.motion_goals._conditions.items(), self.monitors._conditions.items()
        ):
        ...
        eq, neq, eqd, neqd, lin_weight, quad_weight = (
            god_map.motion_statechart_manager.get_constraints_from_tasks()
        )
        god_map.qp_controller.init(
            degrees_of_freedom=self.get_active_free_symbols(eq, neq, eqd, neqd),
        ...
        free_variables = list(
            sorted(
                [
                    ...
                    if v.symbols.position in symbols
                ],
    ...
    def step(self):
        ...
        god_map.world.apply_control_commands(
            next_cmd,
            god_map.qp_controller.config.control_dt,
            god_map.qp_controller.config.max_derivative,
        )
    ...
    def plot_traj(self, plot_kwargs: dict, plot_legend: bool = True):
        ...
        god_map.debug_expression_manager.raw_traj_to_traj(
            control_dt=god_map.qp_controller.config.control_dt
        ).plot_trajectory(
            "",
            sample_period=god_map.qp_controller.config.control_dt,
            filter_0_vel=False,
